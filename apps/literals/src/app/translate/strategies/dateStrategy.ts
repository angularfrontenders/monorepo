...
export class DateStrategy implements ITranslateStrategy {
    ...
    public apply(index: number, text: string, value: any): string {
        if (!this.isDate(value)) {
            console.log(`TranslatePipe: value at position ${index} is not a date object`);
            return text;
        }
        ...
        return text.replace(
            regDate,
            this._datePipe.transform(value, dateParameters.format, dateParameters.timezone, dateParameters.locale)
        );
    }

    private getRegExp = (index: number) => new RegExp(`{${index}:d(?<parameters>\\[.*\\])?}`, 'gm');

    private isDate = (value: any) => Object.prototype.toString.call(value) === '[object Date]';
    ...
    const parameters: string = parametersExp.groups.parameters.replace(/[\[\]']+/g, '');
        ...
        // Get format
        const regFormatExp: RegExpExecArray = /f(ormat)?:(?<format>([^;])*)/.exec(parameters);
        if (regFormatExp && regFormatExp.groups.format) {
            dateParameters.format = regFormatExp.groups.format;
        }
        // Get timezone
        const regTimezoneExp: RegExpExecArray = /t(imezone)?:(?<timezone>([^;])*)/.exec(parameters);
        if (regTimezoneExp && regTimezoneExp.groups.timezone) {
            dateParameters.timezone = regTimezoneExp.groups.timezone;
        }
        // Get locale
        const regLocaleExp: RegExpExecArray = /l(ocale)?:(?<locale>([^;])*)/.exec(parameters);
